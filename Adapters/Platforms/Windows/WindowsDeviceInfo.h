...
namespace GamepadCore::Adapters::Platforms::Windows {
	struct FWindowsDeviceInfo {
		void Detect(std::vector<FDeviceContext>& Devices)
		{
			...
			for (int DeviceIndex = 0; SetupDiEnumDeviceInterfaces(
				 DeviceInfoSet, nullptr, &HidGuid, DeviceIndex, &DeviceInterfaceData);
			     DeviceIndex++)
			{
				...
				if (SetupDiGetDeviceInterfaceDetail(DeviceInfoSet, &DeviceInterfaceData,
								    DetailDataBuffer, RequiredSize, nullptr,
								    nullptr))
				{
					const HANDLE TempDeviceHandle = CreateFileW(
					    DetailDataBuffer->DevicePath, GENERIC_READ | GENERIC_WRITE,
					    FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, NULL,
					    nullptr);
					...
					if (TempDeviceHandle != INVALID_HANDLE_VALUE)
					{
						...
						if (HidD_GetAttributes(TempDeviceHandle, &Attributes))
						{
							     ...
							     Attributes.ProductID == 0x09CC))
							{
								...
								if (HidD_GetProductString(TempDeviceHandle, DeviceProductString,
											  260))
								{
									if (DevicePaths.find(DeviceIndex) == DevicePaths.end())
									{
										std::string FinalString =
										    std::filesystem::path(DetailDataBuffer->DevicePath)
											.string();
		...
		void Write(FDeviceContext* Context)
		{
			...
			if (!WriteFile(Context->Handle, Context->BufferOutput, OutputReportLength,
				       &BytesWritten, nullptr))
			{
			}
		}

		bool CreateHandle(FDeviceContext* DeviceContext)
		{
		...
		EPollResult PollTick(HANDLE Handle, unsigned char* Buffer,
							 std::int32_t Length,
							 DWORD& OutBytesRead)
		{
		...
		bool PingOnce(HANDLE Handle, std::int32_t* OutLastError)
		{
		...
		void ProcessAudioHapitc(FDeviceContext* Context)
		{
			...
			if (!WriteFile(Context->Handle, Context->BufferAudio, BufferSize,
				       &BytesWritten, nullptr))
			{
